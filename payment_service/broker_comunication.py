...
from hipotap_common.queues.payment_queues import AUTHORIZE_PAYMENT_QUEUE
...
def broker_requests_handling_loop():
    ...
    subscriber.subscribe_to_queue(
        AUTHORIZE_PAYMENT_QUEUE, on_authorize_payment_request
    )
...
def on_authorize_payment_request(ch, method, properties, body):
    ...
    animation = [".","..","...","...."]
    for i in range(5):
        print(f"Simulating payment authorization process{animation[i % len(animation)]}")
        time.sleep(1)
